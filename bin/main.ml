...
let rec format_json = function
  | `Assoc pairs ->
    let formatted_pairs = List.map (fun (key, value) ->
...
let () =
  ...
  match args with 
    ...
    | [|_; arg|] when String.length arg > 3 && String.sub arg 0 2 = ".[" && arg.[String.length arg - 1] = ']' && is_integer (String.sub arg 2 ((String.length arg) - 3)) ->
    let index_str = String.sub arg 2 ((String.length arg) - 3) in
    (try ignore (int_of_string index_str) with
    | Failure _ -> Printf.printf "Invalid index: %s\n" index_str; exit 1);
  ...
  | [|_; arg|] when String.length arg > 3 && String.sub arg 0 2 = ".[" && arg.[String.length arg - 1] = ']' ->
